...
| **Avant le premier `admin_societe` réel** *(L1-02c)* | **L'ENRÔLEMENT DU SECOND FACTEUR — qui l'active, et qui peut le retirer ?** RG-DRO-05 le rend obligatoire pour `admin_plateforme`, `admin_societe` et `direction` ; aucun chemin ne l'active aujourd'hui, et `utilisateur_modification` réserve l'écriture à l'administrateur. Le principe est posé par l'exploitation le 07/09/2026 : *un compte peut modifier ce qui parle de LUI ; il ne peut jamais modifier ce qui gouverne SON ACCÈS, et la séparation doit être structurelle, pas conventionnelle.* Les mesures sont rendues (voir le compte rendu de session) ; la forme reste à trancher |
