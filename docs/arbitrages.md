...
| **Avant le premier `admin_societe` réel, ET avant toute écriture d'un chemin d'enrôlement** *(L1-02c, puis D58)* | **LE PLANCHER DE LA TROISIÈME CATÉGORIE DE I1 — `session`, `compte`, `verification`, `second_facteur`, `journal_acces`.** Le principe de l'enrôlement est **tranché par D58** : une seule transition ouverte, dans un seul sens, et le retrait passe par L7-01. Ce qui reste ouvert est ce que la mesure du 07/09/2026 a trouvé sous la question : **les cinq tables techniques ne portent aucune RLS, aucune politique, et le rôle applicatif y détient tous les verbes.** Mesuré sous un contexte de `technicien` — suppression de n'importe quelle ligne de `second_facteur` toutes sociétés confondues, lecture d'une empreinte de mot de passe et d'un jeton de session. Conséquence immédiate : **un cliquet sur `mfa_actif` serait décoratif, et il n'a donc PAS été posé.** Deux options à trancher — traiter `second_facteur` seule en nommant pourquoi les quatre autres attendent, ou donner son régime à toute la troisième catégorie, ce qui est un changement de rang constitutionnel. **Ce que ça coûte à dormir :** rien ne fuit aujourd'hui — aucune route n'expose ces tables — mais la base ne défend rien, si bien que la première requête écrite sans filtre, dans n'importe quel ticket, rendra tout. **Second point, non décidé lui non plus :** `/two-factor/enable` et `/two-factor/disable` sont exposés par l'attrape-tout mais répondent `Unauthorized` — l'intergiciel de session du greffon lit l'identité jointe à la session, ce que la politique de L1-02c filtre (témoin : la session est bien en base, la jointure rend 0). La désactivation en libre-service est donc fermée **par effet de bord**, et se rouvrira sans bruit ; et la transition d'enrôlement ne peut pas s'écrire sur ce point d'entrée. Point annexe et bon marché : les codes de secours sont stockés **en clair** (`storeBackupCodes` non configuré) |
