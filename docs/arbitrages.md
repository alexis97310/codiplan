...
**`getSession` rendait NULL pour tout compte fraîchement connecté** — donc `obtenirSession`, donc toute page authentifiée. Personne ne s'en était aperçu : aucune page ne s'en sert encore. Témoin : deux lignes de session bien présentes en base, la jointure `session × utilisateur` sans désignation rendant zéro.

La cause n'est pas celle qu'on suppose. Better Auth lit la session avec `join: { user: true }`, et Prisma rend cela par **deux instructions SQL pour une seule opération de client** : l'enveloppe ne voyait jamais d'opération `utilisateur`. D'où la branche ajoutée à `utilisateur_lecture` — **le jeton de session désigne aussi son identité**, et il ne rend jamais plus que ce que l'appelant savait.
