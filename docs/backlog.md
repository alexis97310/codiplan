...
**La demande a été REFORMULÉE par l'exploitation le 07/09/2026, et la reformulation est la clé.** « Cloisonner les identités par la société » demandait à la mauvaise couche de répondre à la mauvaise question : **l'authentification PRÉCÈDE la société**, et chercher « existe-t-il un compte pour ce courriel » se fait à un moment où aucune société n'est connue et ne peut l'être. Deux lectures avaient été confondues, et il faut les séparer :
— la **vérification d'identifiants** — à partir d'un courriel, trouver une ligne et comparer une empreinte. Elle ne rend jamais d'identité à personne, et elle porte **déjà en entrée** la seule ligne qu'elle a le droit de voir ;
— la **lecture d'identités** — nom, courriel, rôle d'autrui. Opération de locataire, à cloisonner.
La politique compose donc une **branche d'authentification** — la ligne exactement désignée par une variable que le chemin d'authentification pose lui-même — et une **branche locataire**. Ni rôle passe-partout, ni `SECURITY DEFINER`, ni branche « sans société, tout est ouvert ».

*Trois mesures demandées avant arbitrage, jouées le 07/09/2026 sur base jetable, sous le rôle applicatif, politique réellement posée (témoin : `relrowsecurity` et `relforcerowsecurity` constatés vrais, et zéro ligne vue sans variable).*
**(1) LA BORNE TIENT.** Variable nommant un courriel : **1 ligne vue, exactement celle-là**. Une AUTRE ligne nommément demandée : **0**. Par identifiant : **0**. Balayage `LIKE '%@iso.test'` : **1**. Par `OR` sur deux courriels : **1**. Variable vide : **0**. Aucun contexte : **0**. Témoin de non-vacuité : le propriétaire voit 16 lignes, et la ligne refusée existe.
**(2) LA VARIABLE NE SURVIT PAS — à condition d'être posée EN TRANSACTION.** Mesuré : `set_config(…, false)` **persiste** sur la connexion et serait lu par la requête suivante d'un autre utilisateur — le danger est réel ; `set_config(…, true)` meurt au `COMMIT` **et** au `ROLLBACK`. **L'adaptateur de Better Auth n'a pas à être déformé** : une extension Prisma sur `utilisateur` place chaque lecture dans sa propre transaction, qui pose la variable tirée du `where` de la requête. Mesuré de bout en bout : connexion réelle **aboutit**, courriel inexistant **refusé**, variable **absente** de la connexion après coup. Le tracé montre que l'authentification ne touche `utilisateur` que par **trois désignations** — par courriel (`findFirst`), par identifiant (`findUnique`, pour `mfa_actif`), et par une **relation imbriquée depuis `session`**. **Cette troisième casse**, et c'est NOTRE code, pas Better Auth : `lib/auth/connexion.ts:114` exige la relation, Prisma lève « Field utilisateur is required to return data, got null ». La séparation en deux lectures — la session par son jeton, l'identité par l'identifiant que l'appelant tient déjà — est mesurée et fonctionne.
**(3) L'INDISCERNABILITÉ TIENT.** Courriel inexistant et mot de passe faux rendent le **même message**. Et le témoin décisif : ce que l'appelant observe est **inchangé** — 1 ligne pour un courriel connu, 0 pour un inconnu, exactement comme sans politique. La politique n'ajoute rien à ce qu'un tiers peut distinguer.
**UNE QUATRIÈME QUESTION EST APPARUE, ET ELLE N'ÉTAIT PAS DEMANDÉE.** La politique n'ayant qu'un `USING`, PostgreSQL en fait valoir l'expression en `WITH CHECK` : **la création de compte est REFUSÉE** sous le rôle applicatif (mesuré — « Failed to create user »). Une identité neuve n'a ni habilitation ni variable qui la nomme. Poser une RLS sur cette table gouverne donc aussi les ÉCRITURES, ce que la décision ne disait pas. **Qui crée une identité, et sous quel contexte, est une question de droits — elle reste ouverte.**

**Ne se fusionne pas seule** : L1-02b et L1-02c voyagent ensemble, la clause de `utilisateur_client` étant inerte tant que `app.client_id` n'est posée par personne.
*Relu contre les sources citées le 07/09/2026 — empreinte `7e63421a`.*
