...
*Mesuré le 07/09/2026, base jetable, rôle applicatif, forme posée puis retirée :* société B ne voit plus que ses 2 identités au lieu des 24 de la plateforme — **le cloisonnement demandé fonctionne**. Un utilisateur interne et un `admin_societe` en voient 12, celles de leur société. **Deux résultats bloquent, et ils ne se tranchent pas dans un ticket.**
**(1) L'AUTHENTIFICATION S'ARRÊTE.** Sans contexte société — le chemin de Better Auth, qui lit `utilisateur` par le client applicatif hors transaction — la lecture rend **zéro identité**, et la recherche par courriel ne trouve rien. Les trois échappatoires connues coûtent chacune quelque chose : un rôle PostgreSQL dédié à l'authentification est une **seconde clé passe-partout** (ce que D38 range précisément comme telle) ; une fonction `SECURITY DEFINER` est **refusée par un gardien statique** dont la liste d'exceptions est close et vide (D50) ; une branche « pas de société active » dans la politique **rouvre tout**, ce qui annule la décision. Le choix touche D34, D39 et D50 : il appartient à l'arbitrage.
**(2) UN COMPTE PORTAIL LIRAIT LES IDENTITÉS DU PERSONNEL DE LA SOCIÉTÉ.** Mesuré : 10 identités, dont `admin_societe`, `direction`, `adv`, `technicien`. La branche `utilisateur_societe` est de forme « société » et ne connaît pas `app.client_id`. Le compte portail ne voit PAS l'identité du compte portail d'un autre client — la forme « habilitation » de L1-02b tient —, mais il voit le personnel. Restreindre cette branche est possible ; c'est une décision de droits, pas une modalité.
*Acceptation :* ce que voient un compte portail, un compte interne et un `admin_societe` est mesuré et rendu AVANT toute fusion ; l'authentification est éprouvée sur le chemin réel de Better Auth, pas sur une requête fabriquée ; le gardien d'exhaustivité de D41 range `utilisateur` dans sa nouvelle catégorie ; `TABLES_SANS_RLS` la perd et le gardien de `rls-declaree` le constate.
...
*Relu contre les sources citées le 07/09/2026 — empreinte `3524feaf`.*
