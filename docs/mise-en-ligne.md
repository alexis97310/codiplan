...
14. **Les jours fériés, une fois la première AGENCE créée dans l'application** : même flux, case `etendre_feries`, seule cochée. Elle ne prend aucun territoire : il est lu sur les agences (D46). Sans agence, le geste refuse en le disant — *« Aucun territoire n'est rattaché à une agence »* — et n'invente rien. **Ce bouton ne sert pas encore sur une base hébergée, et c'est mesuré** (22/09/2026, base neuve migrée par un propriétaire non superutilisateur, une agence en base) : le script lit `agence` sous cloisonnement forcé, sans identité exemptée, voit zéro ligne et refuse avec la même phrase — le §9 du 07/09. Tant que `scripts/lib/feries.ts` n'est pas réparé, le geste se joue depuis un terminal sous un rôle exempté des politiques (`HORIZON_DATABASE_URL=… pnpm feries:etendre`), puis `pnpm feries:horizon` constate les douze mois.
