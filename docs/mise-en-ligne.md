...
3 bis. **Créer le rôle `codiplan_app` AVEC son mot de passe** depuis la console SQL de votre base neuve (§1, la commande `CREATE ROLE … NOBYPASSRLS`), **puis déposer deux secrets DANS LE DÉPÔT** : `PRODUCTION_MIGRATION_DATABASE_URL` et `PRODUCTION_DATABASE_URL` (§2). Sans eux, le geste 4 refuse de partir — *et c'est pour cela que le rôle vient AVANT : le second secret porte son mot de passe. Mesuré le 22/09/2026 sur une vraie installation, où la note disait l'inverse et bloquait ici.*
4. **Appliquer les migrations depuis GitHub**, en six clics, sans terminal (§7). La migration trouve le rôle en place et lui accorde ses droits.
...
Le rôle `codiplan_app` est créé par la migration `20260820130000_force_rls_role_applicatif` **s'il n'existe pas encore** (`IF NOT EXISTS`, ligne 51), et **sans mot de passe**. Sur la base de démonstration, un mot de passe lui a été posé à la main hors dépôt, et c'est lui qui vit dans le secret `DATABASE_URL` du dépôt.

**Sur votre base NEUVE, ce mot de passe n'existe pas encore, et personne ne peut le poser à votre place.** ~~Après le geste 4 — la migration crée le rôle —, ouvrez la console SQL de votre hébergeur et jouez `ALTER ROLE codiplan_app WITH LOGIN PASSWORD …`.~~ **Cette phrase était fausse, et elle bloquait au geste 4 — mesuré le 22/09/2026 sur une vraie installation** : le flux de migration exige les DEUX secrets `PRODUCTION_*` avant de partir, dont `PRODUCTION_DATABASE_URL`, qui porte ce mot de passe ; il ne pouvait donc pas être posé « après ». **Le rôle se crée D'ABORD, avec son mot de passe, AVANT la migration** — ouvrez la console SQL de votre hébergeur, sur la base neuve, et jouez :

```sql
CREATE ROLE codiplan_app LOGIN PASSWORD '<un mot de passe long, tiré au hasard>'
  NOSUPERUSER NOBYPASSRLS NOCREATEDB NOCREATEROLE NOINHERIT;
```

Ce sont les attributs exacts que la migration lui donnerait ; elle trouvera le rôle en place, sautera sa création et se contentera de lui **accorder ses droits** — c'est ce qui s'est passé le 22/09. **`NOBYPASSRLS` n'est pas décoratif** : c'est l'attribut qui soumet ce rôle aux politiques de cloisonnement. Un rôle `BYPASSRLS` verrait **toutes les sociétés** à travers la connexion de l'application, la garde de `lib/db/garde-role.ts` le refuserait au démarrage, et le contrôle de cloisonnement du flux de migration rougirait — mais entre les deux, rien ne cloisonnerait. (Si le rôle existe déjà sur le cluster — c'est le cas d'un second déploiement sur le même hébergeur —, `ALTER ROLE codiplan_app WITH LOGIN PASSWORD '…' NOBYPASSRLS;` suffit.)

C'est ce mot de passe qui compose la `DATABASE_URL` de l'hébergeur d'application **et** le secret de dépôt `PRODUCTION_DATABASE_URL` — les deux se déposent **ensuite**, et la migration vient **après**. *La note précédente disait « on recopie le secret » : c'était vrai de la démonstration et faux de la production, et c'est le genre de phrase qui fait brancher une production sur une base de démonstration sans que rien ne s'en aperçoive.*
...
13. **Le taux horaire n'est pas posé par le geste d'amorçage — il a SON geste**, séparé (décision du 09/09, construit le 10/09), **et depuis le 22/09/2026 sa propre CASE dans le flux « Amorcer une base »** (AMORCAGE-2) : cocher `poser_taux`, renseigner `societe` (l'identifiant rendu au geste 9 bis) et `montant` — **entier dans l'unité la plus fine de la devise de la société** (D68 : `7000` pour 7 000 XPF hors taxes chez CODIMA NC ; `6500` pour 65,00 EUR), `date_effet` facultative, à défaut le jour du geste dans le fuseau de la société. **Le montant n'a aucune valeur par défaut, et le flux refuse de partir s'il est vide** : c'est un prix (CLAUDE.md §8). Le résumé de l'exécution **répète le taux formaté — le relire** : le geste refuse ensuite de rejouer, et une erreur d'échelle se corrige par le chemin ordinaire. Depuis un terminal, la commande reste `TAUX_INITIAL_CONFIRME=oui pnpm tsx scripts/taux-initial.mts --societe <uuid> --montant 7000 [--date AAAA-MM-JJ]`. Tant qu'aucune ligne de `taux_horaire` n'existe, RG-TAR-04 n'a rien à appliquer et aucune intervention ne se valorise. **Ce geste ne pose que le PREMIER taux** (TAUX-1, 22/09/2026) : une hausse, une correction ou tout taux qui SUCCÈDE au premier passe désormais par l'écran `/parametres/taux-horaire` (capacité `parametrer_societe`), qui relit le montant formaté à l'écran de confirmation avant d'écrire, et ne modifie ni ne supprime jamais une ligne passée.
14. **Les jours fériés, une fois la première AGENCE créée dans l'application** : même flux, case `etendre_feries`, seule cochée. Elle ne prend aucun territoire : il est lu sur les agences (D46). Sans agence, le geste refuse en le disant — *« Aucun territoire n'est rattaché à une agence »* — et n'invente rien. ~~**Ce bouton ne sert pas encore sur une base hébergée, et c'est mesuré**~~ — **RÉPARÉ le 22/09/2026 (FERIES-1)**. Le défaut, mesuré le même jour sur une base neuve migrée par un propriétaire non superutilisateur, une agence en base : le script lisait `agence` sous cloisonnement forcé, sans identité exemptée, voyait zéro ligne et refusait avec la même phrase — le §9 du 07/09. Il lit désormais les agences sous une **identité exemptée des politiques** — le rôle connecté s'il l'est, sinon un rôle exempté dont il est membre, pris par `SET LOCAL ROLE` le temps d'une transaction de lecture, exactement comme l'inventaire à chaque migration — et **refuse de conclure à défaut**, en nommant le rôle, la base et la variable (`HORIZON_DATABASE_URL`). Le flux n'exige donc aucun secret de plus : l'URL de migration qui permet l'inventaire permet ce geste. Rejoué sous les rôles d'un hébergeur : 36 fériés ajoutés, puis `pnpm feries:horizon` constate les douze mois. Deux refus distincts, et ils ne se ressemblent plus : *« Aucun territoire n'est rattaché à une agence »* veut dire que la table est **réellement** vide ; *« … est soumis aux politiques de cloisonnement »* veut dire que le rôle de migration n'a accès à aucun rôle exempté lisant les tables — auquel cas l'inventaire post-migration refuse déjà pour la même raison, et c'est lui qu'il faut avoir réparé d'abord. L'arbitrage — identité exemptée plutôt que contexte posé société par société — est écrit avec sa condition de réouverture dans `scripts/lib/feries.ts`.
...
**Sur une base NEUVE, il n'y a rien de plus à faire :** la première migration crée le rôle applicatif `codiplan_app` **s'il n'existe pas — et vous l'aurez créé au geste 3 bis, avec son mot de passe, si bien qu'elle lui accorde seulement ses droits** —, les suivantes posent les tables, les politiques de cloisonnement et les déclencheurs d'audit.
...
**Et c'est ici que se trouvent les identifiants de société dont le geste 7 a besoin.** L'étape *Inventaire à plat* les imprime, sous la forme `CODE (uuid)`. Sur une base de production neuve, l'inventaire ne nomme **aucune** société : il n'y en a pas encore, et c'est le point suivant — voir §9, *« et la société, d'où vient-elle ? »*. **Cette phrase était vraie du document et fausse du code jusqu'au 22/09/2026** : mesuré ce jour-là à 11 h 50 sur la base neuve d'Alexis, l'étape *Inventaire à plat* rougissait sur *« aucune société en base : le seed n'a pas produit le socle attendu »* — un contrôle écrit pour la démonstration, où le seed vient de tourner. L'inventaire lit désormais la cible que le flux a retenue (`CIBLE_RETENUE`) : sur `production`, zéro société est un état ; sur la démonstration, c'est toujours un écart (AMORCAGE-2, `tests/unit/db/inventaire.test.ts`). Le journal de l'étape dit quel socle il a exigé.
...
**Les JOURS FÉRIÉS ne sont dans aucun des deux**, et ce n'est pas un oubli : leur horizon est **glissant** et se calcule **par territoire**, donc depuis les agences (D46). Le geste existe déjà : `pnpm feries:etendre` une fois qu'une agence existe, puis `pnpm feries:horizon` pour constater les douze mois. **Sans lui, le planning proposerait des créneaux un 1ᵉʳ mai.** *Depuis le 22/09/2026 (AMORCAGE-2), le même flux porte deux cases de plus — `poser_taux` et `etendre_feries` —, chacune sous sa propre case, et un gardien exige que toute case déclarée ait son étape (`tests/unit/ci/gestes-amorcage.test.ts`). La case des fériés avait une limite MESURÉE sur base hébergée, écrite au §6, geste 14 — refermée le jour même (FERIES-1).*
