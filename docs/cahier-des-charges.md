...
| RG-TAR-05 | Une intervention est valorisée au forfait, au temps passé, ou au forfait plus les heures excédentaires. Le mode est fixé à la qualification et modifiable jusqu'à la clôture. **Le temps d'intervention est arrondi AU QUART D'HEURE SUPÉRIEUR**, et **la main-d'œuvre facturée ne peut être inférieure à UNE HEURE** au taux en vigueur. **Arrondi puis plancher s'appliquent UNE SEULE FOIS, sur l'intervention ENTIÈRE**, jamais tâche par tâche — et par intervention, jamais sur le total d'une journée : cinq passages de cinq minutes font **5 heures**, et non 30 minutes. Le plancher ne s'applique **ni au forfait** (le prix d'un forfait ne dépend pas de la durée), **ni au trajet**, **ni au travail interne** — aucun des trois n'est facturé à l'heure. Une intervention étalée sur deux jours reste **une** intervention : un seul arrondi, un seul plancher. **Le plancher s'applique PAR INTERVENTION, sans exception** : deux interventions le même jour sur le même site déclenchent **deux planchers**, y compris si la seconde achève la première. *(amendée par D57, D83, D89)* |
...
| Réf | Règle |
|---|---|
| RG-INT-01 | Une intervention est rattachée à un client et à un site. ~~Elle porte au moins une machine, **sauf pour les types `expertise`, `installation` et `recensement`**. Si la machine n'existe pas, elle est créée avant de démarrer.~~ **AUCUNE MACHINE N'EST EXIGÉE POUR DÉMARRER** *(D120)* : *une intervention peut porter sur autre chose qu'un équipement — un réseau d'air comprimé, par exemple.* Elle PEUT en porter une ou plusieurs (`intervention_machine`), et **aucune machine rattachée signifie le site entier**. Si la machine existe et qu'on veut la nommer, elle se rattache ; si elle n'existe pas, elle se crée — mais ni l'un ni l'autre ne conditionne le démarrage, que le compteur du technicien déclenche d'un seul geste. *(amendée par D16, D120)* |
...
| RG-INT-07 | Le temps de trajet est saisi séparément et **n'est JAMAIS facturé au temps** : le déplacement se facture par un **forfait conditionné par zone** (RG-TAR-06), un seul par intervention ; une ligne de temps de type `trajet` n'est jamais facturable. ~~n'est facturé que selon la règle du contrat, du forfait ou du barème applicable~~ *(amendée par D74 — la version d'origine laissait un contrat facturer le trajet au temps)* |
| RG-INT-08 | Une intervention hors horaires d'ouverture porte automatiquement la majoration paramétrée pour la société. **LA MAJORATION PAIE LA CONTRAINTE D'UN CRÉNEAU POSÉ HORS OUVERTURE, PAS LES MINUTES EFFECTIVEMENT TRAVAILLÉES** *(D108)* : le prorata se calcule sur la part du **CRÉNEAU** qui tombe hors ouverture, jamais sur les minutes réelles — *le client a fait bloquer la soirée d'un technicien ; qu'il finisse tôt ne rend pas la soirée disponible.* Taux, assiette et calendrier de référence restent ceux de D12. *(amendée par D108)* |
...
| RG-PLA-03 | Un chevauchement sur un même technicien est **refusé**, à la pose comme au déplacement : pour un exploitant, deux interventions au même moment sur la même personne sont une promesse qu'on ne peut pas tenir. Deux créneaux qui se **touchent** ne se chevauchent pas. Une intervention **annulée** n'occupe plus rien ; une intervention **clôturée** occupe toujours — elle a eu lieu. Le refus est prononcé **côté serveur** et nomme son motif. *(amendée par D99 — la version d'origine disait « signalé mais reste possible », et le dépôt refusait déjà depuis R2-19.)* |
...
| RG-PLA-05 | ~~Le temps de trajet inter-sites est intégré au calcul de charge.~~ **Le trajet compté est l'ALLER vers le PREMIER site de la journée et le RETOUR depuis le DERNIER** *(D107)* : la colonne ne porte que des distances **depuis l'agence**, et *soustraire deux distances à un point commun n'est pas une distance* — **le temps entre deux sites n'est donc pas compté, et l'application le DIT au lieu de l'approximer.** Le point de départ est l'agence du **TECHNICIEN**, pas celle du site. La phrase d'origine est barrée et non effacée : elle contredisait D56, écrit dans la même règle. La valeur saisie dans `site.temps_trajet_min` **fait foi** quand elle existe ; l'estimation à partir de la zone géographique n'est qu'un **défaut**, appliqué en son absence. Cette valeur est le trajet **depuis l'agence de rattachement du site** (`site.agence_id`) : un site dépend d'une agence et d'une seule, et le temps de trajet **perd son sens si ce rattachement change sans être revu** — la base le refuse. **C'est une donnée de PLANIFICATION et rien d'autre** : elle sert au calcul de charge et aux tournées, elle ne s'ajoute jamais aux heures facturées — le déplacement se facture par forfait (RG-INT-07). *(amendée par D23, D56, D74, D107)* |
| RG-PLA-06 | Une absence validée bloque le créneau ; les interventions posées repassent en file à planifier avec alerte. Tant que l'effectif est d'un seul technicien, l'absence déclenche une alerte de rupture de service et propose le report groupé. **L'effectif se compte PAR AGENCE** *(D106)* — le nombre de techniciens **actifs capables de servir l'agence du site**, parce que c'est lui qui dit s'il existe une alternative ; compter par société ferait taire l'alerte à Koné parce que Ducos a du monde. **Et le report groupé REND les interventions à la file, ensemble : il ne propose AUCUN créneau** *(D106)* — un moteur qui propose sur un effectif d'un ne propose rien, et le planificateur sait ce que le système ne saura jamais. *(amendée par D106)* |
| RG-PLA-07 | Une intervention ne se pose et ne se déplace que **dans le calendrier de l'agence visée** — celle de l'intervention, déduite de son site (RG-PLA-05). L'union des calendriers affichée en vue semaine est un **repère**, jamais un droit de poser. Une agence **sans calendrier refuse** : « inconnu » n'est pas « ouvert », et poser sans horaire connu promettrait un rendez-vous que personne ne peut tenir (I7). Le refus est prononcé **côté serveur** et nomme son motif. *(amendée par D99 — règle introduite par cet arbitrage.)* |
| RG-PLA-08 | Une intervention dont le **client** est **inactif** ne s'affiche plus, par défaut, ni dans le planning ni dans le registre `/interventions`. Le registre porte une case **« Inclure les clients inactifs »** qui revient sur ce choix ; le planning ne l'offre pas. **Ce que la règle NE cache PAS** : la fiche du client continue de montrer l'intégralité de son historique d'interventions quel que soit son statut, et le statut du **site** n'est pas concerné — un site inactif d'un client actif reste affiché. *(amendée par D129 — règle introduite par cet arbitrage.)* |
...
**intervention — LES DEUX TEMPS** *(D120, 15/09/2026)*. `temps_reel_min` est **renommée `temps_valide_min`** et rejointe par **`temps_mesure_min`**, **`temps_valide_par`** et **`temps_valide_le`**. *Le compteur du technicien est la seule source du temps* : `temps_mesure_min` est la somme des segments fermés, écrite par le compteur et **jamais modifiable** — un déclencheur refuse toute autre valeur. `temps_valide_min` est ce qu'un responsable ou l'ADV arrête à la validation, **par défaut égal au mesuré**, et c'est **lui seul** qui entre dans D83. **Le renommage n'est pas cosmétique** : le temps « réel » est le mesuré, et c'est le validé qui se valorise — un nom qui dit le contraire de ce que la colonne porte est la faute que le §9 nomme sur `code_winpro`. *Pourquoi DEUX colonnes : un compteur oublié fausse les indicateurs, et sans elles on ne peut pas voir l'écart.* L'auteur et la date voyagent ensemble, la base l'exige (`intervention_validation_tracee`, posée **NOT VALID** — les interventions clôturées avant ce jour n'ont jamais été validées, et personne ne peut leur inventer un auteur).

**segment_travail** — `societe_id`, **intervention**, **auteur**, début, fin (nullable). *Ajoutée le 15/09/2026 par R5-02, sur **D119**.* **Le compteur du technicien fait foi pour le temps** : cette table porte ce qu'il a mesuré, et le temps facturé s'en déduit — arrondi au quart d'heure supérieur puis plancher d'une heure, une seule fois sur l'intervention entière (RG-TAR-05, D83, D89). **DES SEGMENTS, ET JAMAIS UN SEUL COUPLE DÉBUT/FIN** : *une pause est un fait, et un compteur qui ne garde que le total ne peut pas dire ce qui s'est passé entre-temps* — huit heures entre le premier départ et le dernier arrêt peuvent n'être que quatre heures de travail. **L'arrondi n'est pas ici** : la table stocke des instants, la valorisation arrondit — *arrondir à l'écriture ferait deux lectures d'un même critère, et la seconde déciderait du prix.* `fin` nulle veut dire **« il tourne encore »**, jamais « on ne sait pas » : un index partiel n'autorise qu'**un** segment ouvert par personne — *une personne ne travaille pas à deux endroits à la fois* —, tandis que deux segments **fermés** peuvent se recouvrir, I5 voulant que le travail terrain ne soit jamais perdu. L'auteur n'est pas forcément le technicien affecté : *un renfort d'une heure est un fait, et l'affectation n'en dit rien.* **Aucune suppression n'est possible** — ni privilège, ni politique : *un temps qui peut disparaître sans trace ne fait foi de rien* ; une erreur se corrige par modification, et le journal d'audit garde la valeur d'avant. Forme « **interne** » (D94) : aucun compte de portail ne la lit — *combien de temps une personne nommée a passé chez un client, minute par minute, est une information sur cette personne*, et ce qu'un client a le droit de savoir est le temps **facturé** de **son** intervention.
...
**vgp_verification** — `societe_id`, **machine**, date de vérification, **organisme**, référence du rapport, **origine de l'information**, document facultatif. *Ajoutée le 12/09/2026 par **D114**.* **CODIPLAN n'affirme jamais la conformité** (D88) : cette table enregistre **ce qu'un organisme agréé a écrit**, et aucune colonne ne porte de verdict — le seul calcul du produit est une date. **L'origine est obligatoire et sans défaut** : *un rapport reçu de l'organisme, une vignette photographiée par un technicien et une parole du client au téléphone n'ont pas la même valeur le jour d'un contrôle*, et elle **ne se reconstitue pas après coup**. Énumération close en base ; ses quatre valeurs sont une **proposition**, le vocabulaire d'exploitation appartenant à Alexis. La date qui compte est celle de la **vérification**, jamais de la saisie. Forme « filiation », parent `machine`.

**vgp_observation** — `societe_id`, **vérification**, libellé, **intervention engendrée** (nullable). *Ajoutée le 12/09/2026 par D114, sur D88 §10.* Une observation d'organisme est *un travail à faire, daté, sur une machine identifiée* : **elle a exactement la forme d'une intervention `a_planifier`**, et c'est le seul point où le lot 9 alimente le planning. `intervention_id` est **nulle tant que personne ne l'a planifiée** — *une observation non traitée est l'état qui compte.* Le libellé est du **texte libre** : une énumération d'observations serait une classification que CODIPLAN n'a pas le droit de faire. Forme « filiation », parent `vgp_verification` — **l'adossement se chaîne** jusqu'à `machine`.

**vgp_campagne** — `societe_id`, **famille**, date d'ouverture, date de clôture (nullable). *Ajoutée le 12/09/2026 par D114, sur D88 §8.* **Faire passer une famille de « non soumise » à « soumise » n'ouvre pas deux cents alertes : cela ouvre un objet unique, daté, avec un reste-à-faire visible.** **Le compteur n'est PAS une colonne** : il se dérive des machines de la famille sans information depuis l'ouverture — *un compteur stocké se désynchronise en silence, et un compteur figé est pire qu'une alerte de trop, parce qu'il a l'air de mesurer.* Elle ne se clôt pas toute seule à zéro : *une campagne close est une décision*, et le compteur peut remonter. Forme « société » et non « filiation » : son parent serait `famille_materiel`, de forme « ascendance », et une campagne ne doit pas hériter du refus de remonter vers une famille dont aucune machine n'est visible.
