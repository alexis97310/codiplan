...
**Un import ne DÉPLACE pas un site d'une agence à l'autre** (R6-01), et c'est une limite nommée plutôt qu'un oubli. D56 refuse de changer `agence_id` sans revoir `temps_trajet_min` ; le gabarit n'expose pas cette colonne (L1-09). _Passer le rattachement quand même a été mesuré : le refus lève, et la levée emporte le lot entier_ — une ligne d'agence mal remplie ferait perdre trois cents fiches. Une ligne qui nomme une autre agence met donc ses **autres** colonnes à jour sans déplacer le site, et le rattachement se change à l'écran. _Condition de levée : le jour où L1-09 lira la colonne « Temps de trajet »._

`lib/imports/parc-cibles.ts` (R6-01) répond à une question que `parc-clients.ts`, `parc-agences.ts` et `parc-familles.ts` ne posent pas. **Le parent et la cible ne sont pas la même chose** : les trois premiers disent _« que désigne cette cellule ? »_, celui-ci dit _« cette fiche existe-t-elle déjà ? »_. L'index des clients est le seul qui réponde aux deux — un client n'a pas de parent —, et c'est ce qui a rendu la confusion invisible. La confondre ne produit aucune erreur : _elle produit des DOUBLONS_, une fiche neuve à chaque import, que seul le second passage du même fichier révélerait.

Et l'ambiguïté y est **possible**, contre l'intuition : _mesuré le 16/09/2026_, `site` ne porte **aucun** index unique sur (client, libellé), là où `modele_materiel` et `prestation` en ont un. Une première rédaction lui en prêtait un ; **un scénario d'`upsert` l'a démentie en une exécution**, la relecture ne l'avait pas vue.
...
~~**Un seul type va jusqu'au bout, et l'écran le dit.**~~ **QUATRE DES CINQ VONT JUSQU'AU BOUT DEPUIS R6-01** — clients, sites, modèles, prestations. La phrase est barrée et non effacée : elle a gouverné ce module, et ce qui a été décidé un jour se relit. Les **contacts** restent au rapport seul, et le motif est mesuré plutôt qu'annoncé : `lib/contacts/` ne porte que `saisie.ts`, sans aucun dépôt — c'est L1-03b qui l'écrira. _Un écran qui accepterait un fichier de contacts en montrerait le rapport et ne saurait rien en faire_ : le bouton ne s'affiche donc pas, et **son absence est motivée à l'écran**, jamais tue.

Le drapeau n'est pas cru sur parole : `tests/unit/imports/types-dimport.test.ts` lit `lib/imports/` et exige que les types marqués complets soient **exactement** ceux qui portent une fonction `appliquerLeLotDe<Type>`. Depuis R6-01 il exige trois choses de plus, contre des sources qu'aucun des fichiers gardés ne contrôle : que `APPLICATIONS ∪ SANS_APPLICATION` soit **exactement** les gabarits publiés, que chaque application ait son **annulation**, et que chaque type applicable ait un **index de cible**.
...
**LE PLANNING N'EST PLUS LE MÊME POUR TOUT LE MONDE (R5-01).** La matrice du §5.2 donne `consulter_planning` en **restreint** au technicien depuis l'origine, et _rien ne la lisait_ : son seul appelant était son propre scénario. `lib/interventions/perimetre-technicien.ts` la lit, et les trois lectures du dépôt — le planning, la fiche, la ligne brute — appliquent le même fragment de filtre, écrit **une seule fois**. Trois verdicts et jamais un booléen : complet, restreint à sa propre identité, **aucun** — et ce dernier REFUSE au lieu de rendre zéro ligne, _un planning vide se lisant « il n'y a rien » là où il faut lire « ce n'est pas pour vous »_.

**Et ce que la base ne tient pas est ÉCRIT plutôt que tu.** La forme « parc » répond « quelle société, quel client, quels sites », jamais « quelle personne » : la restriction vit donc dans la couche applicative. Ce n'est pas une seconde lecture d'un critère que la politique porte déjà — _rien en base ne répond à cette question_ —, et `tests/isolation/planning-du-technicien.test.ts` le montre en lisant **deux** interventions sous le contexte cloisonné du technicien avant que le dépôt n'en retranche une. Porter la restriction en base ajouterait une quatorzième forme de politique : c'est un arbitrage, pas un ticket.
...
**Les plages et les jours travaillés se règlent depuis l'application (R3-13, 14/09/2026).** Ils s'affichaient sur `/parametres/agences` et ne se modifiaient **nulle part** : le seul chemin était le semis ou une console. C'était loin d'être un confort — un calendrier d'agence décide de quatre choses mesurables (le refus à la pose, le départ du compteur d'accusé de réception, le dénominateur du taux d'occupation, l'assiette de la majoration hors ouverture), et _une agence qui ouvre le samedi ne pouvait pas le déclarer._ Le tableau mène désormais à la fiche d'un calendrier, où chaque jour porte ses plages ; **le pas, lui, n'a pas bougé de la ligne**, parce qu'on le règle en regardant celui des autres établissements.

**« Fermer un jour » n'est pas une case à cocher.** La table le dit depuis sa naissance : _pas de booléen « ouvert », deux sources pour un même fait finissent par se contredire._ Un jour sans plage EST un jour fermé — fermer, c'est retirer ses plages ; ouvrir, c'est lui en donner une.

**Deux états interdits, tenus PAR LA BASE et non par Zod seul.** Deux plages qui se recouvrent compteraient deux fois les mêmes heures ouvrables — donc un taux d'occupation faux, _sans que rien ne le dise_ ; une plage plus courte que le pas rend une grille **vide**, c'est-à-dire un jour affiché comme ouvert sur lequel le planning ne propose rien. Trois déclencheurs les refusent, et le second verrou joue **dans les deux sens** : raccourcir une plage sous le pas est refusé, élever le pas au-dessus de la plus courte plage aussi. _Le second sens est celui qu'on oublie_ — le pas se règle dans la ligne, la plage sur la fiche, et n'en garder qu'un laisserait l'autre produire l'état qu'on interdit. **Deux plages qui se TOUCHENT ne se chevauchent pas** : 08:00–12:00 et 12:00–17:00 sont la journée coupée par le déjeuner.

**Ce que le réglage change RÉTROACTIVEMENT est écrit à l'écran, faute de pouvoir l'empêcher.** Rien de ce qui est posé ne bouge — la pose décide au moment de la pose, et le départ du compteur d'accusé est matérialisé (D85). Mais le dénominateur du taux et l'assiette de la majoration relisent les plages **à chaque rendu** : fermer un samedi ce soir change un taux lu la semaine dernière. _Rien n'est matérialisé, donc le travail est de le DIRE là où le réglage se fait._ La moitié qui touche un montant facturé est portée à l'arbitrage : elle n'appartient pas à une session.
