...
describe("connexion et session serveur", () => {
  ...
  it("ouvre une session en base, sans société active", async () => {
    ...
    // Sous le PROPRIÉTAIRE, et c'est une conséquence assumée de L1-02d :
    // `session` se désigne par son JETON, jamais par l'identifiant de son
    // compte — un identifiant n'est pas un secret. Un DÉNOMBREMENT par compte
    // n'est donc pas un chemin de production, c'est une observation du harnais.
    const avant = await clientOwner().session.count({
      where: { utilisateur_id: id },
    });

    await auth.api.signInEmail({ body: { email, password: MOT_DE_PASSE } });

    const sessions = await clientOwner().session.findMany({
      where: { utilisateur_id: id },
