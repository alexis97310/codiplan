...
| **désignation** | la ligne que l'appelant NOMMAIT DÉJÀ, **plus** un rattachement à la société active | `utilisateur` SEULE — et uniquement pour ce qui PRÉCÈDE la société *(L1-02c)* | `utilisateur` |
...
**Elle ne vaut QUE pour les opérations qui précèdent le contexte de locataire**, et le motif d'addition recevable est unique et étroit : *l'opération se produit avant qu'une société soit connue, et par construction ne peut pas l'être.* « C'est plus simple ainsi » et « on posera le contexte plus tard » ne sont pas des motifs — ce sont des descriptions du code. Liste close à une entrée, gardée dans les deux sens : `TABLES_DESIGNATION`.
