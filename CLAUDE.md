...
|---|---|---|
| `docs/constitution/erreurs-a-ne-pas-refaire.md` | Le §9 **intégralement** — 58 entrées datées, chacune sous son titre depuis DOC-1 ; le sommaire en tête retrouve une entrée déjà lue, il ne dispense d'aucune des 58 | **Avant d'écrire un gardien** ; avant de croire qu'une réparation en est une ; avant d'affirmer un état sans l'avoir observé |
| `docs/constitution/organisation-du-code.md` | Le §6 — le paragraphe d'introduction (`(prévu)`) **intégralement**, puis, depuis DOC-1, la seule section du répertoire visé (`### \`app/\``, `#### \`lib/db/\``, …) | Avant de créer un module ou un fichier dans `lib/` : ouvrir le sommaire, puis sa section. Pour chercher où une règle vit déjà, le sommaire seul suffit souvent |
...
(22/09/2026), le sommaire des fichiers qui dépassent 250 lignes contre leurs propres
titres — un titre sans entrée de sommaire, ou une entrée qui pointe dans le vide, rougit.
